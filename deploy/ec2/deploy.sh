...
# bash -s reads the deployment script from stdin; do not let Compose consume it.
docker compose run --rm --no-deps -T --interactive=false backend npx prisma migrate deploy </dev/null
