...
export function DebatePage() {
  ...
  useEffect(() => {
    ...
    if (socket.connected && !didJoin.current) { didJoin.current = true; socket.emit('join_room', { roomId, userId, username, password }); }
    return () => { socket.off('connect', onConnect); socket.emit('leave_room'); resetRoom(); socket.disconnect(); };
