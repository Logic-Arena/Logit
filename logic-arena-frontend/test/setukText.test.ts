...
const replacements = [
  ...
  ['네이버와 비교함.', '국내 기업과 비교함.'],
  ['정보처리기사로 역량을 인증함.', '정보처리기사로 역량을 인증함.'],
  ['메가학원에서 학습함.', '메가학원에서 학습함.'],
  ['대학원과 영어학원을 비교함.', '대학원과 영어학원을 비교함.'],
  ['toeic은 시험임.', 'toeic은 시험임.'],
  ['ＴＯＥＩＣ을 응시함.', 'ＴＯＥＩＣ을 응시함.'],
...
test('사교육과 수상은 이름을 일반화하여 통과시키지 않는다', () => {
  const { result } = applyForbiddenFilter('메가학원에서 학습함. 대회에서 대상을 수상함.');
  assert.equal(result, '메가학원에서 학습함. 대회에서 대상을 수상함.');
  assert.deepEqual(checkForbidden(result), ['메가학원', '대상']);
});
