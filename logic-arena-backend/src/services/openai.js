...
export async function generateSoloFeedback({ topic, essaySide, essayText, structuredArgumentEnabled = true }) {
  ...
  if (structuredArgumentEnabled) {
    ...
    // 자유 서술형 모드: 섹션 라벨 없이 자연스러운 피드백
    prompt =
      `논술 주제: "${topic}"\n\n` +
      `학생이 선택한 입장: ${stance}\n` +
      `학생의 글을 반드시 ${stance} 입장을 기준으로 읽고, 다른 입장으로 바꾸도록 유도하지 마세요.\n\n` +
      `학생이 작성한 주장문:\n${essayText}\n\n` +
      `당신은 논술 지도 교사입니다. 학생이 자유롭게 작성한 이 글을 읽고, 다음 평가 관점에 따라 개선 제안을 해주세요:\n` +
      `1. 주장의 명확성: 학생의 입장이 명확하게 드러나는가?\n` +
      `2. 근거의 타당성: 주장을 뒷받침하는 근거가 논리적이고 타당한가?\n` +
      `3. 예시의 적절성: 근거를 구체화하는 예시가 효과적인가?\n` +
      `4. 예상 반론 대비: 반대 입장의 반론을 예상하고 대응했는가?\n` +
      `5. 재반론 논리: 반론에 대한 답변이 설득력 있는가?\n\n` +
      `각 관점에 대해 "무엇이 좋았고, 어떻게 고치면 더 나아지는지" 자연스럽게 서술하세요.\n` +
      `섹션 라벨(【주장】, 【근거】 등)을 쓰지 말고, 학생이 작성한 문단을 직접 인용하거나 가리키며 피드백하세요.\n` +
      `예: "첫 문장에서 밝힌 주장은~", "두 번째 문단에서 제시한 근거는~"\n` +
      `정답을 알려주지 말고, 학생 스스로 퇴고하도록 유도하는 질문·제안형 문구로 작성하세요.\n\n` +
      `다음 JSON 형식으로 답변하세요 (주석 제외, 순수 JSON만):\n` +
      `{\n` +
      `  "claim": "주장의 명확성에 대한 피드백 1~2문장 (자연스러운 서술형)",\n` +
      `  "evidence": "근거의 타당성에 대한 피드백 1~2문장 (자연스러운 서술형)",\n` +
      `  "example": "예시의 적절성에 대한 피드백 1~2문장 (자연스러운 서술형)",\n` +
      `  "counterArgument": "예상 반론 대비에 대한 피드백 1~2문장 (자연스러운 서술형)",\n` +
      `  "rebuttal": "재반론 논리에 대한 피드백 1~2문장 (자연스러운 서술형)",\n` +
      `  "overall": "전체 주장문에 대한 총평 1~2문장"\n` +
      `}\n\n` +
      `반드시 위 JSON 형식만 출력하고 다른 문구는 추가하지 마세요.`;
  }
...
export async function judgeSoloEssay({ topic, playerName, essaySide, essayText }) {
  ...
  const prompt =
    ...
    `advice(개인 조언) 작성 규칙:\n` +
    `- 3~4문장\n` +
    `- 제출문에서 잘한 점 1가지와, summary에서 이미 지적한 것과는 다른 개선할 점 1가지를 구체적으로 지적\n` +
    `- summary가 총점 관점의 강점/약점 요약이라면, advice는 그와 겹치지 않는 실행 가능한 다음 단계여야 함 (예: 문장 구조, 근거 제시 방식, 반론 대응 전략 등 summary가 다루지 않은 측면)\n` +
    `- 실제 논리나 표현을 인용해서 서술\n` +
